...
// pick from array those rows where list has an element equal to
...
// rows in out.   The values are copied from self to other.
// This is designed for repeated sampling.
...
// will be accepted) from file nameed filename. If filename is the
// empty string, "" then the data will be read from stdin. namex is
// the name of the matrix created. isColor is a bool passed in and
// returns the whether the file was in ppm (color which is P3 or P6)
// or pgm (gray-scale which is P2 or P5). If grayscale, a matrix will
// be created with the numeric value of each pixel. If color, RGB will
// be read in as 3 consecutive numbers.
